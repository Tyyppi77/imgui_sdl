...
namespace ImGuiSDL
{
	...
	// TODO: Hide implementation details from the user.
	struct Device
	{
		...
		std::map<UniformColorTriangleKey, TriangleCacheItem> UniformColorTriangleCache;
		...
		std::map<GenericTriangleKey, TriangleCacheItem> GenericTriangleCache;
	...
	struct Rect
	{
		...
		bool UsesOnlyColor(const Texture* texture) const
		{
			...
			// TODO: There is IO->Fonts->TexUvWhitePixel;
	...
	Device::TriangleCacheItem DrawTriangleWithColorFunction(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3, const FixedPointTriangleRenderInfo& renderInfo, const std::function<Color(float x, float y)>& colorFunction)
	{
		...
		if (renderInfo.MaxX - renderInfo.MinX == 0 || renderInfo.MaxY - renderInfo.MinY == 0) return Device::TriangleCacheItem{ nullptr };
		...
		SDL_Texture* cache = CurrentDevice->MakeTexture(renderInfo.MaxX - renderInfo.MinX, renderInfo.MaxY - renderInfo.MinY);
		...
		return Device::TriangleCacheItem{ cache, renderInfo.MinX, renderInfo.MinY, renderInfo.MaxX - renderInfo.MinX, renderInfo.MaxY - renderInfo.MinY };
	}
	...
	void DrawTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3, const Texture* texture, const Rect& bounding)
	{
		...
		const auto& cached = DrawTriangleWithColorFunction(v3, v2, v1, renderInfo, [&](float x, float y) {
		...
		CurrentDevice->GenericTriangleCache[key] = cached;
	}
	...
	void DrawUniformColorTriangle(const ImDrawVert& v1, const ImDrawVert& v2, const ImDrawVert& v3)
	{
		...
		// The naming inconsistency in the parameters is intentional. The fixed point algorithm wants the vertices in a counter clockwise order.
		const auto& cached = DrawTriangleWithColorFunction(v3, v2, v1, renderInfo, [&color](float, float) { return color; });
		...
		CurrentDevice->UniformColorTriangleCache[key] = cached;
	}

	void DrawRectangle(const Rect& bounding, const Texture* texture, const Color& color)
	{
		...
		else
		{
			...
			SDL_RenderCopy(CurrentDevice->Renderer, texture->Source, &source, &destination);
		}
	}
	...
	void Initialize(SDL_Renderer* renderer, int windowWidth, int windowHeight)
	{
		...
		ImGuiSDL::Texture* texture = new ImGuiSDL::Texture();
				...
				else
				{
					...
					for (unsigned int i = 0; i + 3 <= drawCommand->ElemCount; i += 3)
					{
						...
						if (i + 6 <= drawCommand->ElemCount)
						{
							...
							{
								DrawRectangle(bounding, texture, Color(v0.col));
